...
z_result_t z_array_uint8_decode(z_iobuf_t* buf) {
  ...
  z_result_t r;
  r.tag = ARRAY_UINT8;
  z_result_t r_vle = z_vle_decode(buf);
  if (r_vle.tag != VLE) {
    r.tag = ERROR;
    r.value.error = VLE_PARSE_ERROR;
    return r;
  }  
  r.value.a_uint8.length = (unsigned int)r_vle.value.vle;  
  r.value.a_uint8.elem = z_iobuf_read_n(buf, r.value.a_uint8.length); 
  return r;
}
...
z_result_t z_accept_decode(z_iobuf_t* buf, uint8_t header) {
  ...
  z_result_t r_cpid = z_array_uint8_decode(buf);
  ...
  z_result_t r_bpid = z_array_uint8_decode(buf);
  ...
  r.value.accept.client_pid = r_cpid.value.a_uint8;
  r.value.accept.broker_pid = r_bpid.value.a_uint8;
